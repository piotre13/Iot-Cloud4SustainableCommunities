...
while True:
    ...
    if command in ["new_sens", "new_act", "start_sim", "quit"]:
        if command == "start_sim":
            if not sensor_list:
                print("No sensors available to simulate. Please add sensors first.")
                continue
            try:
            ...
            controller = Controller(TH=20, rooms=rooms)  # Set a default threshold
